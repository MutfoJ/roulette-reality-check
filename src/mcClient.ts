 ...
 */
export function runMonteCarloInWorker({ runs, iterations, starting, opts, onProgress, signal }: RunMcArgs): Promise<MonteCarloSummary> {
  return new Promise<MonteCarloSummary>((resolve, reject) => {
    ...
    const req: McRequest = { type: "run", runs, iterations, starting, opts };
